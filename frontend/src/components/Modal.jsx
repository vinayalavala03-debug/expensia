...
const Modal = ({children, isOpen, onClose, title}) => {
  if (!isOpen) return null  // Modal renders only when open
      ...
      <div className='relative p-4 w-full max-w-2xl max-h-full'>
        ...
        <div className='relative bg-white rounded-lg shadow-sm dark:bg-white'>
          ...
          <div className='flex items-center justify-between  p-4 md:p-5 border-b rounded-t dark:border-gray-600 border-gray-200'>
            ...
            <button
              type='button'
              className='text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-200 rounded-lg text-sm w-8 h-8 inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white cursor-pointer'
              onClick={onClose}
            >
              <svg
                className="w-3 h-3"
